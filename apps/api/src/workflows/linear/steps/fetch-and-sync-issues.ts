...
export async function fetchAndSyncIssues({
  ...
  const issueBatch = await Promise.all(
  ...
  if (isTuple(issueBatch)) {
    await db.batch(issueBatch);
  }

  const eventBatch = issues.nodes.map((issue) => {
    ...
    return db
      ...
      .values({
        ...
        projectId: null,
        userId: issue.creatorId ?? null,
        teamId: null,
        payload: issue as any,
        webhookId: null,
