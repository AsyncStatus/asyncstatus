    ...
    () => [
      ...
      },
      {
        ...
        connectLink: getLinearIntegrationConnectUrl({
          ...
          redirectUri: `${import.meta.env.VITE_API_URL}${linearIntegrationCallbackContract.url()}`,
          organizationSlug: params.organizationSlug,
        }),
        onDisconnect: () => {
        ...
        settingsChildren: (
          <div className="space-y-6">
              ...
              {linearUsers.data && linearUsers.data.length > 0 && (
                <div className="text-sm text-muted-foreground space-y-2">
                  {linearUsers.data.map((user) => {
                    const member = organizationMembers.data?.members.find(
                      (m) => m.linearId === user.userId,
                    );

                    return (
                      <div key={user.id} className="flex items-center gap-2">
                        <span>{user.displayName || user.name || user.email || user.userId}</span>
                        <ArrowRight className="size-4" />
                        <Select
                          value={member?.id}
                          onValueChange={(value) => {
                            const selected = organizationMembers.data?.members.find(
                              (m) => m.id === value,
                            );
                            if (!selected) return;
                            updateMemberMutation.mutate({
                        ...
                        >
                          ...
                          <SelectContent>
                            {organizationMembers.data?.members.map((m) => (
                ...
                <div className="text-sm text-muted-foreground space-y-1">
                  {linearProjects.data.map((project) => (
                    <div key={project.id} className="flex items-center gap-2">
                      ...
                      {project.state && <Badge variant="outline">{project.state}</Badge>}
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                Resync users, teams and projects
              </Button>
            </div>
          </div>
        ),
        children: (
          <div className="space-y-4">
            <div className="space-y-2">
              <h4 className="font-medium">What this integration does</h4>
              <ul className="text-sm text-muted-foreground space-y-1 ml-4 list-disc">
                <li>Automatically tracks your Linear issues, projects and progress.</li>
                <li>Generates meaningful status updates from product work.</li>
            ...
            <div className="space-y-2">
              <h4 className="font-medium">Privacy & Security</h4>
              <ul className="text-sm text-muted-foreground space-y-1 ml-4 list-disc">
                <li>Read-only access for data processing where possible.</li>
            ...
            <div className="rounded-lg bg-muted p-3 text-sm text-muted-foreground">
              <strong>Status Example:</strong> "Closed 8 issues, progressed 2 projects, and
              completed the sprint goals."
            </div>
          </div>
        ),
      },
