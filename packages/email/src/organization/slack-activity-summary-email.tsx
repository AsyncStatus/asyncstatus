...
export function SlackActivitySummaryEmail(props: {
  preview: string;
    ...
    <Layout preview={props.preview}>
      ...
      <Section>
        ...
        <Heading as="h2" style={h2}>
          Slack activity summary for <strong>{props.organizationName}</strong>.
        </Heading>
        ...
        {props.channelSummaries.length > 0 && (
          <Section style={{ marginBottom: 32 }}>
            <Heading as="h3" style={{ ...h2, fontSize: 18, marginBottom: 12 }}>
              #️⃣ Channel Highlights
            </Heading>
            {props.channelSummaries.map((summary, index) => (
              <Text
                key={`slack-summary-${index}-${summary.content.slice(0, 20)}`}
...
SlackActivitySummaryEmail.PreviewProps = {
  preview: "Your Slack activity summary is ready",
  recipientName: "John Doe",
  organizationName: "AsyncStatus",
  generalSummary: "Increased coordination around the release; minor risk from an infra incident.",
  channelSummaries: [
    { content: "#shipping: Finalized release scope; action items assigned to QA and DevOps" },
    { content: "#incidents: Investigated staging outage; fix deployed; postmortem drafted" },
  ],
  effectiveFrom: "2024-01-15T00:00:00Z",
  effectiveTo: "2024-01-22T00:00:00Z",
  viewUpdatesLink: "https://app.asyncstatus.com/acme/activity/slack",
};

export default SlackActivitySummaryEmail;
